...
class MessageController {
  ...
  async createMessage(req, res) {
    ...
    if (req.body.channel !== 'DEFAULT') {
      return res.status(400).json('Bad input - expected req.body.channel to be DEFAULT.');
    }

    if (req.body.event === undefined || req.body.event === null || req.body.event === '') {
      return res.status(400).json('Bad input - expected req.body.event to be non empty.');
    }

    if (typeof req.body.event !== 'string' && !(req.body.event instanceof String)) {
      return res.status(400).json('Bad input - expected req.body.event to be a string.');
    }
    let parsedData;
    try {
      parsedData = JSON.parse(JSON.stringify(req.body.data));
    } catch (e) {
      return res.status(400).json('Bad input - expected req.body.data to be valid JSON.');
    }
